...
if ($result->num_rows > 0) {
    ...
    case 'cambiarEstado':
        ...
        case 'crearAsistencia':
            ...
            if ($id_programador) {
                // Actualiza la consulta SELECT para calcular 'horas_trabajadas'
                $sql = "SELECT 
                         fecha, 
                         id_docente,
                         TIMEDIFF(hora_salida, hora_inicio) AS horas_trabajadas                   
                         FROM programador
                ...
                if ($result->num_rows > 0) {
                    // Actualiza la consulta INSERT para usar 'horas_trabajadas'
                    $insert_sql = "INSERT INTO asistencias (fecha, id_docente, horas_trabajadas) 
                                   VALUES (?, ?, ?)";
                    ...
                    while ($row = $result->fetch_assoc()) {
                        $fecha = $row['fecha'];
                        $horas_trabajadas = $row['horas_trabajadas'];  // Ahora usamos 'horas_trabajadas'
                        $id_docente = $row['id_docente'];
        
                        // Actualiza el tipo de parámetro para 'horas_trabajadas'
